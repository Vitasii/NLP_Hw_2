...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        trlen = random.randint(4, max(max_len,4))
        _doc = document[:trlen]

        mask_len = random.randint(1, max(1,trlen // 2))
        pre_len = random.randint(0, trlen - mask_len)

        prefix = _doc[:pre_len]
        masked = _doc[pre_len:pre_len + mask_len]
        suffix = _doc[pre_len + mask_len:]

        full_string = (
            prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked
            + self.PAD_CHAR * (self.block_size - 1 - len(prefix) - len(suffix) - len(masked))
        )
